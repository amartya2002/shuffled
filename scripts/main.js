document.addEventListener("DOMContentLoaded", () => {
    ...
    const getSelectedCategoryfromInput = () => {
    ...
    const populateCategoryDropdown = () => {
      const uniqueCategories = [
        ...new Set(questions.map((questionObject) => questionObject.category)),
      ];
      categorySelect.innerHTML = '<option selected="All">All</option>';
    ...
    let selectedCategory = ""; // Initialize selectedCategory as an empty string
    let filteredQuestions = [];
    let shuffledQuestions = [];
    let questionIndex = 0;
   
    const categorySelect = document.getElementById("categorySelect"); // Assuming your dropdown has an id of "categorySelect"
  
    categorySelect.addEventListener("change", () => {
        selectedCategory = categorySelect.value; // Update selectedCategory based on the dropdown value
    
        if (selectedCategory === "All") {
            filteredQuestions = questions; // If "All" is selected, use all questions
        } else {
            // Filter questions based on the selected category
            filteredQuestions = questions.filter(questionObject => questionObject.category === selectedCategory);
        }
    ...
    const generateRandomQuestion = () => {
      if (shuffledQuestions.length === 0) {
        questionDisplay.textContent =
          "All questions shown. Click 'Generate' to repeat.";
        return;
      }
    ...
    addQuestionForm.addEventListener("submit", (event) => {
      ...
      const category = getSelectedCategoryfromInput();
      if (newQuestion !== "") {
